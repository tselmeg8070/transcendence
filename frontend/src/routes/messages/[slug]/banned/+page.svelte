<script lang="ts">
	...
	import DiscordAvailableBan from "$components/chat/discord_available_ban.svelte";
	import DiscordBanned from "$components/chat/discord_banned.svelte";
...
<div class="mx-7 w-full">
	<p class="mt-5 text-base font-semibold uppercase">Banned</p>
	<div class="overflow-y-scroll no-scrollbar max-w-full scrollbar-hide mt-3">
		{#if !loading}
			{#if channel.bans.length === 0}
				<div>No one is banned</div>
			{/if}
			{#each channel.bans as ban}
				<DiscordBanned banInfo={ban} channel={channel} handleSent={update}/>
			{/each}
		{/if}
	</div>
	...
	<div class="overflow-y-scroll no-scrollbar max-w-full scrollbar-hide">
		{#if !loading}
			{#each channel.members.filter(e => {
				if (channel.bans.find(c => e.id === c.userId))
					return (false);
				...
				<DiscordAvailableBan user={user} channel={channel} handleSent={update}/>
			{/each}
		{/if}
	</div>
</div>
